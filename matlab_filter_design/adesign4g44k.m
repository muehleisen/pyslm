...
% tolerance for Type 1 filters from S1.42
ftol= [10, 12.5 16 20, 25 31.5 40, 50 63 80, 100 125 160, 200 250 315, ...
...
for N=Nlist
    ...
    [bd4,ad4]=invfreqz(hs2,pi*fs2/max(fs2),Na,Nb,wt,10,0.001);
    ...
    if errba<errmin
        ...
        subplot(2,1,2)
     	semilogx(fs,bidiff,'r', fs, fulldiff, 'k', ftol, up1, 'b:',ftol, -low1, 'b:' )
        grid
...
astring = ['a = ['];
for I=1:(Na+1)
    astring = strcat(astring, sprintf('%#1.15g,',ad4(I)));
end
astring(end)=']';
...
bstring = ['b = ['];
for I=1:(Nb+1)
    bstring = strcat(bstring, sprintf('%#1.15g,',bd4(I)));
end
bstring(end)=']';
disp(astring)
disp(bstring)
...
% disp('From bilinear')
% disp(sprintf('a = [%#1.15g,%#1.15g,%#1.15g,%#1.15g,%#1.15g]',abi))
% disp(sprintf('b = [%#1.15g,%#1.15g,%#1.15g,%#1.15g,%#1.15g]',bbi))
