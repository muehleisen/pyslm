...
% 1109, 2064, 2075, 3341, 3377, 3876, 5047, 7051

% new weights that reduces HF weights  yields: 3281
errmin=1;
errba=1;
...
Nlist = [2064, Nmin:Nspace:Nmax]

       % 2064,  1393,  2992, 3732, 3733, 3884, 5555, 6267, 6604, 7305, 7736, 8505, 8938, 10799,10902,11746,12353]

% Nlist=[1109, 2064, 3368, 3377, 3876, 5047, 7051]
Nlist = 2992
for N=Nlist
    [N,errba,Nmin,errmin]
    ...
    if errba<errmin
        ...
        semilogx(fs,20*log10(abs(hb)),'g',fb,20*log10(abs(hs)),'r',fd4,20*log10(abs(hd4)),'k')
     	axis([20,fmax,-70,+5])
        legend('bilinear','analog','invfreqz')
